...
export default function FavoritesPage(){
  ...
  useEffect(() => {
    api.get('/user/me/favorites')
       .then(r => setFavorites(r.data.results.map(x => x.word)))
  ...
  return (
    <Box>
      ...
      {favorites.length === 0 ? <Typography>Nenhuma palavra favoritada</Typography> :
        favorites.map(w => <WordCard key={w} word={w} onOpen={openWord} favorited onToggleFavorite={toggleFavorite} />)}
    </Box>
  );
}
